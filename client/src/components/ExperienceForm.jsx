                    ...
                    {data.map((experience, index) => (
                        <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                            ...
                            <div className="grid md:grid-cols gap-3">
                                <input onChange={(e) => updateExperience(index, 'company', e.target.value)} value={experience.company || ''} type="text" placeholder="Company Name" className="px-3 py-2 text-sm rounded-lg" />
                                <input onChange={(e) => updateExperience(index, 'position', e.target.value)} value={experience.position || ''} type="text" placeholder="Job Title" className="px-3 py-2 text-sm rounded-lg" />
                                <input onChange={(e) => updateExperience(index, 'start_date', e.target.value)} value={experience.start_date || ''} type="month" className="px-3 py-2 text-sm rounded-lg" />
                                <input onChange={(e) => updateExperience(index, 'end_date', e.target.value)} disabled={experience.is_current} value={experience.end_date || ''} type="month" className="px-3 py-2 text-sm rounded-lg disabled:bg-gray-100" />

                            </div>
                            <label className="flex items-center gap-2">
                                <input onChange={(e) => { updateExperience(index, 'is_current', e.target.checked ? true : false); }} className="rounded border-gray-300 text-blue-600 focus:ring-blue-500" type="checkbox" checked={experience.is_current || false} />
