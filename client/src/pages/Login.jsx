...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Show a friendly inline message instead of a browser alert() popup
    ...
    return (
        <div className='flex items-center justify-center min-h-screen bg-gray-50'>
            <form onSubmit={handleSubmit} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
                ...
                <button type="submit" disabled={loading} className="mt-2 w-full h-11 rounded-full text-white bg-green-500 hover:opacity-90 transition-opacity disabled:opacity-60">
                    {loading ? 'Please wait...' : (state === "login" ? "Login" : "Sign up")}
                </button>
                <p onClick={() => { setState(prev => prev === "login" ? "register" : "login"); setError('') }} className="text-gray-500 text-sm mt-3 mb-11">{state === "login" ? "Don't have an account?" : "Already have an account?"} <a href="#" className="text-green-500 hover:underline">click here</a></p>
            </form>
        </div>
    )
}
